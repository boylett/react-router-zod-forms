 ...
 */
function templateLiteralPattern (parts: any[]): string {
  return parts
    .map(
      part => {
        ...
        if (def.type === "literal") {
          return `(${ (def.values || []).map((value: any) => escapeRegExp(String(value))).join("|") })`;
        }

        if (def.type === "enum") {
          return `(${ Object.values(def.entries || {}).map((value: any) => escapeRegExp(String(value))).join("|") })`;
        }

        if (def.type === "number") {
          return (def.checks || []).some((check: any) => check?._zod?.def?.format === "safeint")
            ? "-?\\d+"
            : "-?\\d+(?:\\.\\d+)?";
        }
  ...
  if (rest.name) {
    ...
    if (shape) {
      ...
      if (shape.def.type === "template_literal" && "parts" in shape.def && !("pattern" in rest)) {
        Object.assign(rest, {
          pattern: templateLiteralPattern(shape.def.parts as any[]),
        });
      }
      ...
      for (const check of shape.def.checks || []) {
        ...
        if ("check" in check._zod.def && check._zod.def.check === "mime_type" && "mime" in check._zod.def && !("accept" in rest)) {
          ...
          Object.assign(rest, {
            accept: (check._zod.def.mime as string[]).join(","),
          });
        }
      }
